...
fun evaluarMano(mano: Array<Carta>): String {
    ...
    return when {
        ...
        cantidades ==listOf(valores.last() ==14) -> "Carta Alta"
        else -> "Carta Alta"
    }
}
...
fun main() {
    ...
    val ganador = when {
        ranking.indexOf(jugadaJugador) > ranking.indexOf(jugadaComputadora) -> "Ganaste"
        ranking.indexOf(jugadaJugador) < ranking.indexOf(jugadaComputadora) -> "Perdiste"
        else -> "empate"
    }
    println("\nResultado: $ganador")
}